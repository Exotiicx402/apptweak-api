...
export function useSnapchatHistory() {
  const [data, setData] = useState<SnapchatHistoryData | null>(null);
  ...
  const fetchHistory = useCallback(async (startDate: string, endDate: string, campaignId?: string) => {
    ...
    try {
      const { data: responseData, error: invokeError } = await supabase.functions.invoke("snapchat-history", {
