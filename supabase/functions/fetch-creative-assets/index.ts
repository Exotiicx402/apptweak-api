 ...
 async function downloadAndStoreAsset(
   ...
   try {
     ...
     console.log(`Uploading to: ${storagePath} (${(blob.size / 1024).toFixed(1)} KB)`);
 ...
 // Fetch Meta creatives with full-resolution asset URLs
 async function fetchMetaCreatives(): Promise<MetaCreativeData[]> {
   ...
   try {
     // Fetch ads with their creatives - request video_id for videos
     const adsUrl = new URL(`https://graph.facebook.com/v19.0/${adAccountId}/ads`);
     adsUrl.searchParams.set("fields", "id,name,creative{id,name,object_type,image_url,video_id,object_story_spec}");
     ...
     // Collect video IDs to batch fetch
     const videoIds: string[] = [];
     const videoIdToAdMap = new Map<string, number>();
     ...
     for (const ad of data.data || []) {
       ...
       // Determine if video or image based on multiple signals
     ...
     if (videoIds.length > 0) {
       console.log(`Fetching ${videoIds.length} video sources...`);
       
       for (let i = 0; i < videoIds.length; i += 50) {
         const batch = videoIds.slice(i, i + 50);
         const videoUrl = new URL(`https://graph.facebook.com/v19.0/`);
         videoUrl.searchParams.set("ids", batch.join(','));
         videoUrl.searchParams.set("fields", "id,source,picture");
         ...
         try {
           ...
           if (videoResponse.ok) {
             ...
             for (const [videoId, video] of Object.entries(videoData)) {
               const idx = videoIdToAdMap.get(videoId);
               if (idx !== undefined && creatives[idx]) {
                 const v = video as any;
                 creatives[idx].videoSourceUrl = v.source || null;
                 if (v.picture) {
             ...
             console.warn(`Video batch fetch failed: ${await videoResponse.text()}`);
           }
         } catch (e) {
     ...
     console.log(`Extracted ${creatives.length} Meta creatives`);
   } catch (error) {
 ...
 serve(async (req) => {
   ...
   try {
         ...
         if (fullAssetUrl || posterUrl || thumbnailUrl) {
           const { error: upsertError } = await supabase
             ...
             .upsert({
               ...
               original_url: creative.imageUrl || creative.videoSourceUrl,
     ...
     console.log(`Videos: ${results.videosDownloaded}, Images: ${results.imagesDownloaded}`);
